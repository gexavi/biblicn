...
//
// La BnF ajoute aussi souvent des dates de naissance/mort et un rôle après
// le prénom (ex. "Coben, Harlan (1962-....). Auteur du texte") : tout ce qui
// suit la première parenthèse est retiré, sur le prénom avant de le
// recombiner avec le nom (sinon le nom se retrouverait perdu après le texte
// retiré plutôt qu'avant).
function bnfAuthorToDisplayName(creator) {
  const m = creator.match(/^([^,]+),\s*(.+)$/);
  if (!m) return creator.split('(')[0].trim();
  const surname = m[1].trim();
  const firstName = m[2].split('(')[0].trim();
...
// Note sur 20 : ramène toute valeur saisie (fiche, import CSV) dans les
// bornes attendues plutôt que de stocker tel quel un nombre négatif ou > 20.
